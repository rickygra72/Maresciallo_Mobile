...
# --- FORM DI TRATTAMENTO QUESITO CON FALLBACK AUTOMATICO ANTI-503 ---
st.markdown("---")
prompt = st.text_area("Quesito operativo / Note di verifica", placeholder="Inserisci las tue richieste operative o documenti da esaminare...")
    ...
    if not chiave_pulita:
        ...
        with st.spinner("Analisi in corso ed elaborazione dati..."):
            try:
                ...
                # --- STRATEGIA DI FALLBACK SU PIÙ MODELLI PER PREVENIRE ERRORI 503 ---
                modelli_disponibili = ['gemini-2.5-flash', 'gemini-1.5-flash', 'gemini-1.5-pro']
                res = None
                errore_ultimo = None

                for mod in modelli_disponibili:
                    try:
                ...
                if res and res.text:
                    ...
                    st.error(f"⚠️ I server di elaborazione sono temporaneamente saturi. Riprovare tra pochi secondi. Dettaglio: {str(errore_ultimo)}")
