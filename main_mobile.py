...
PASSWORD_APPLICATIVO = "GdiF_117"  # 👈 MODIFICA QUESTA CON LA TUA PASSWORD DI SQUADRA
...
# --- DA QUI IN POI L'APPLICAZIONE PARTE SOLO SE AUTENTICATI ---
...
# Esteso il supporto esplicito ai tipi MIME nativi delle fotocamere mobile
    ...
    if not entry_key.strip():
        ...
        with st.spinner("Analisi in corso ed incrocio flussi..."):
            try:
                ...
                if allegati:
                    for f in allegati:
                        # Riconoscimento avanzato per flussi da fotocamera mobile (tipo MIME o estensione)
                        tipo_mime = getattr(f, "type", "").lower()
                        est = f.name.split('.')[-1].lower()
                        
                        if "image" in tipo_mime or est in ['png', 'jpg', 'jpeg']:
                            # Forza un tipo valido per le API se lo smartphone manda un formato ambiguo
                            ...
                            immagini_parts.append(
                                types.Part.from_bytes(data=f.read(), mime_type=mime_finale)
                            )
