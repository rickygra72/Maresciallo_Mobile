...
# --- FUNZIONI DI ESPORTAZIONE REPORT CORAZZATE ---
def genera_pdf_report(testo_relazione):
    ...
    for riga in righe:
        if riga.strip().startswith("|"):
            ...
            # Calcolo flessibile degli spazi per evitare il crash 'not enough space'
            riga_tabella = ""
            for i, col in enumerate(colonne):
                if i == 0:
                    riga_tabella += f"{col[:18]:<20}" 
                elif i in [1, 2, 3]:
                    riga_tabella += f"{col[:10]:<12}" 
                else:
                    riga_tabella += f" {col[:25]}"     
            
            # Usiamo multi_cell per evitare blocchi o interruzioni geometriche
            pdf.multi_cell(0, 5, riga_tabella)
        else:
            if in_tabella:
                pdf.ln(3) 
                in_tabella = False
