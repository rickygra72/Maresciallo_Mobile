...
PASSWORD_APPLICATIVO = "GdiF_117"  # 👈 PASSWORD DI SQUADRA BLINDATA AGGIORNATA
...
# --- FUNZIONE DI CATTURA IMMEDIATA ALLEGATI (DALLA ARCHIVIO LOCALE O GALLERIA) ---
    ...
    if "caricatore_chiave" in st.session_state and st.session_state.caricatore_chiave:
        for f in st.session_state.caricatore_chiave:
            if f.name not in st.session_state.dati_allegati_salvati["nomi_file"]:
                ...
                # Gestione robusta per immagini salvate in memoria
...
# Modificato l'invito testuale: si scattano prima le foto per poi caricarle come file stabili
st.file_uploader(
    "📁 Importa Atti, Foto scattate o Documenti", 
