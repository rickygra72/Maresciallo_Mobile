...
with st.expander("🔒 NOTE SULLA SICUREZZA E PRIVACY DEI DATI"):
    ...
    * **Cancellazione Certificata all'Arresto:** Al moment della chiusura della pagina web o tramite la pressione del comando *'Nuova Indagine (Reset)'*, la RAM viene immediatamente sovrascritta e liberata, determinando la distruzione istantanea e irreversibile di tutti i dati aziendali o personali trattati.
...
# --- FORM DI TRATTAMENTO QUESITO CON RETRY AUTOMATICO (ANTI-503) ---
st.markdown("---")
    ...
    if not chiave_pulita:
        ...
        with st.spinner("Analisi in corso ed elaborazione dati..."):
            try:
                ...
                # --- CICLO DI RETRY PER GESTIRE I PICCHI TEMPORANEI (ERRORE 503) ---
                res = None
                tentativi_massimi = 3
                ...
                for tentativo in range(tentativi_massimi):
                    try:
                        ...
                        if "503" in str(err_api) or "UNAVAILABLE" in str(err_api):
                            if tentativo < tentativi_massimi - 1:
                                time.sleep(2)  # Pausa di 2 secondi prima di riprovare
                                continue
                        raise err_api  # Se non è un 503 o siamo all'ultimo tentativo, rilancia l'eccezione
