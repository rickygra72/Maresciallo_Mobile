...
# Libreria professionale per reportistica PDF indistruttibile
...
# --- NUOVA GENERAZIONE PDF INDISTRUTTIBILE CON REPORTLAB ---
def genera_pdf_report(testo_relazione):
    ...
    for riga in righe:
        ...
        # Pulisce i tag HTML o caratteri non interpretabili per evitare errori di markup
        riga_sicura = riga_strip.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;')
        
        if riga_strip.startswith('|'):
            if "---" in riga_strip:
                continue
            story.append(Paragraph(riga_sicura, style_tabella))
        elif riga_strip.startswith('###') or riga_strip.startswith('**'):
...
if st.button("🚀 INVIA RICHIESTA", use_container_width=True):
    ...
    "3. RIGORE A RITMETICO: Quando esegui calcoli, sommatorie o riepiloghi di bilancio, verifica sempre il pareggio e la quadratura contabile. Esponi chiaramente le basi di calcolo (es. Imponibile, Aliquota, IVA, Totale) per garantire la massima verificabilità.\n"
