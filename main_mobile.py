...
# --- FUNZIONE DI GENERAZIONE PDF IN LINEA (SOLUZIONE DEFINITIVA CRASH SPAZIO) ---
def genera_pdf_report(testo_relazione):
    ...
    for riga in righe:
        ...
        # Protezione Tabelle: converte le barre verticali in testo lineare per azzerare gli errori geometrici
        if riga_strip.startswith("|"):
            if "---" in riga_strip:
                continue
            pdf.set_font("Courier", size=9)
            elementi = [el.strip() for el in riga_strip.split("|") if el.strip()]
            riga_testuale = " - ".join(elementi)
            pdf.multi_cell(0, 5, f"• {riga_testuale}")
            continue
            
        # Formattazione Titoli o Evidenziati
        if riga_strip.startswith("###") or riga_strip.startswith("**"):
            pdf.set_font("Helvetica", "B", 11)
            pdf.ln(2)
            pdf.multi_cell(0, 6, riga_strip.replace("#", "").replace("**", "").strip())
            pdf.ln(1)
        else:
            pdf.set_font("Helvetica", size=10)
            # Codifica protetta per evitare caratteri speciali non supportati
            riga_codificata = riga_strip.encode('latin-1', 'replace').decode('latin-1')
...
# --- POPUP / AVVISO IN EVIDENZA PER L'ORIENTAMENTO DELLE FOTO ---
st.warning(
    "📸 **DIRETTIVA ACQUISIZIONE FOTO DA MOBILE:**\n\n"
    "Il sistema accetta correttamente gli scatti effettuati in **modalità verticale**. "
    "A causa delle restrizioni di memoria dei browser smartphone, gli scatti in modalità orizzontale "
    "potrebbero non caricarsi. Se hai necessità di acquisire un'immagine orizzontale, effettua "
    "normalmente lo scatto, aprilo nella galleria del telefono, **fai uno screenshot (cattura schermo)** "
    "e importa lo screenshot nel terminale."
)

# --- DISPOSITIVA PRIVACY & SICUREZZA ESPANSA ---
with st.expander("🔒 DISPOSITIVA DI COMPLIANCE: SICUREZZA ESECUTIVA E PRIVACY DEI DATI"):
    st.markdown("""
...
st.file_uploader(
    "📁 Importa File da Dispositivo (Foto Scattate, Excel, PDF, Word)", 
