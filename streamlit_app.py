...
def carica_dati_da_drive():
    ...
    if rows and len(rows) > 0:
        ...
        # Pulizia forzata immediata e formattazione con puntino delle migliaia
        for col_f in ["Punti_VS", "Donazioni_Tech"]:
            if col_f in df.columns:
                df[col_f] = df[col_f].apply(lambda x: normalizza_valore_vs(x) if col_f == "Punti_VS" else normalizza_valore_tech(x))
                df[col_f] = df[col_f].apply(lambda val: f"{int(val):,}".replace(",", ".") if int(val) > 0 else "0")
...
def salva_dati_su_drive(sheet, df):
    try:
        ...
        if client_fresco:
            ...
            # Rimozione totale e definitiva di qualsiasi puntino, M o K prima di inviare a Drive
            for col_f in ["Punti_VS", "Donazioni_Tech"]:
