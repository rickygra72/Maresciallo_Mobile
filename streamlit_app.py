...
def load_config():
    if os.path.exists(CONFIG_FILE):
        try:
            ...
            pass
    return {
        "tipo_settimana": "SPINTA COMPLETA", "target_push_str": "100M",
        "limite_save_str": "45M", "target_tech_str": "10k",
...
TEXTS = {
    "🇮🇹 🇮🇹 Italiano": {
        ...
        "target_vs_push": "Obiettivo VS Spinta (es. 100M)",
        "limite_save_txt": "Limite di Risparmio (es. 45M)",
        "target_tech": "Target Donazioni Tech (es. 10k)",
        ...
        "m_not_submitted": "ℹ️ Non hai ancora inviato le squadre per這個 mese o sono in fase di compilazione.",
        ...
        * **VS** = Punti VS inseriti  
        * **Comb.** = Combattente Sabato (si/no)  
        * **Tech** = Donazioni Tech inserite  
        * **Stelle** = Premi Stella  
    ...
    "🇩🇪 🇩🇪 Deutsch": {
        ...
        "m_format_info": "💡 Einheit in **Millionen (M)**. Wenn leer gelassen, wird standardmäßig `0M` verwendet.",
...
def parse_compact_number(val_str, default_val=0.0):
    ...
    val_clean = str(val_str).strip().upper().replace('M', '').replace('K', '').replace(',', '.')
    if not val_clean or val_clean == "NAN":
        return default_val
    try:
        val_num = float(val_clean)
        return default_val if np.isnan(val_num) else val_num
    except ValueError:
        return default_val
    ...
    if rows and len(rows) > 0:
        ...
        for c in colonne_richieste:
            if c not in df.columns:
                df[c] = "0" if c in ["Eventi_Totali", "Giorni_Inattivita", "Premi_Stella"] else ("no" if c != "Donazioni_Tech" else "10")
...
def calcola_punteggi_alleanza(df, config_dict):
    ...
    target_push = parse_compact_number(config_dict.get("target_push_str", "100"), 100.0)
    limite_save = parse_compact_number(config_dict.get("limite_save_str", "45"), 45.0)
    target_tech = parse_compact_number(config_dict.get("target_tech_str", "10"), 10.0)
    ...
    for _, row in df.iterrows():
        # Legge esattamente il numero puro inserito, ignorando M o K
