...
TEXTS = {
    "🇮🇹 🇮🇹 Italiano": {
        ...
        "sidebar_btn_deserto": "🏜️ Gestione Iscrizioni Deserto", "sidebar_btn_canyon": "🏔️ Gestione Iscrizioni Canyon",
    ...
    "🇬🇧 🇬🇧 English": {
        ...
        "sidebar_btn_deserto": "🏜️ Desert Registration", "sidebar_btn_canyon": "🏔️ Canyon Registration",
        ...
        "success_drive": "Connected", "month_curr_label": "Month", "month_prev_label": "Prev", "month_edit_title": "Edit",
        "month_analysis_title": "Growth", "ai_prompt_label": "Ask", "ai_send_btn": "Send", "btn_empty_log": "Clear",
        "rule_goal_title": "Goal", "rule_goal_desc": "Desc", "rule_vs_title": "1. VS", "rule_vs_desc": "Desc",
        "rule_ev_title": "2. Events", "rule_ev_desc": "Desc", "rule_tech_title": "3. Tech", "rule_tech_desc": "Desc",
        ...
        "m_submit_btn": "SUBMIT", "m_save_success_approval": "Sent for approval!", "m_save_success_direct": "Saved!",
        "pending_requests_warn": "Pending {count}", "btn_approve": "Approve", "btn_approve_all": "Approve All", "btn_reject": "Reject",
        "opt_yes": "yes", "opt_no": "no", "tipo_squadra_opzioni": ["", "Tanks", "Missiles", "Aircraft", "Mixed"], "db_legend_text": "Legend",
    ...
    "🇩🇪 🇩🇪 Deutsch": {
        ...
        "sidebar_btn_deserto": "🏜️ Wüsten-Anmeldung", "sidebar_btn_canyon": "🏔️ Canyon-Anmeldung",
        ...
        "success_drive": "Verbunden", "month_curr_label": "Monat", "month_prev_label": "Vormonat", "month_edit_title": "Bearbeiten",
        "month_analysis_title": "Wachstum", "ai_prompt_label": "Frage", "ai_send_btn": "Senden", "btn_empty_log": "Leeren",
        "rule_goal_title": "Ziel", "rule_goal_desc": "Desc", "rule_vs_title": "1. VS", "rule_vs_desc": "Desc",
        "rule_ev_title": "2. Events", "rule_ev_desc": "Desc", "rule_tech_title": "3. Tech", "rule_tech_desc": "Desc",
        ...
        "m_submit_btn": "SENDEN", "m_save_success_approval": "Zur Genehmigung gesendet!", "m_save_success_direct": "Gespeichert!",
        "pending_requests_warn": "Wartet {count}", "btn_approve": "Genehmigen", "btn_approve_all": "Alle genehmigen", "btn_reject": "Ablehnen",
        "opt_yes": "ja", "opt_no": "nein", "tipo_squadra_opzioni": ["", "Panzer", "Raketen", "Flieger", "Gemischt"], "db_legend_text": "Legende",
...
def genera_pdf_iscrizioni(df_iscrizioni, titolo_evento, settimana):
    ...
    story.append(Paragraph(f"⚔️ REPORT ISCRIZIONI {titolo_evento} - Settimana: {settimana}", title_style))
...
# --- GESTIONE PANNELLO R4: DESERTO ---
if st.session_state.get("vista_deserto_r4", False):
    st.title("🏜️ Gestione Iscrizioni - Campo di Battaglia del DESERTO (R4/R5)")
    if st.button("🔙 Torna al Pannello Principale"):
        st.session_state.vista_deserto_r4 = False
        st.rerun()
    ...
    if spreadsheet_obj:
        ...
        st.info(f"🗓️ Settimana di riferimento: **{settimana_corrente}**")
        
        # Sincronizzazione Inadempienti (Malus in Mancata_Risposta_R4)
        if not df_drive_punti.empty and not df_isc_d.empty:
            tutti_membri = [str(n).replace("🔴 ", "").strip() for n in df_drive_punti["Nickname"].tolist()]
            iscritti_correnti = df_isc_d[df_isc_d["Settimana"] == settimana_corrente]["Nickname"].tolist() if "Settimana" in df_isc_d.columns else []
            inadempienti = [m for m in tutti_membri if m not in iscritti_correnti]
        ...
        if not df_isc_d.empty:
            df_sett = df_isc_d[df_isc_d["Settimana"] == settimana_corrente].copy() if "Settimana" in df_isc_d.columns else df_isc_d.copy()
            st.subheader("📋 Elenco Iscrizioni Registrate (Deserto)")
            
            # Possibilità di cancellare riga errata
            if not df_sett.empty:
                riga_da_cancellare = st.selectbox("Seleziona iscrizione da cancellare", options=["-- Seleziona --"] + [f"{row['Nickname']} ({row['Scelta']})" for _, row in df_sett.iterrows()])
                if riga_da_cancellare != "-- Seleziona --":
                    nick_selezionato = riga_da_cancellare.split(" (")[0]
                    if st.button("🗑️ Cancella Iscrizione Selezionata"):
                        df_isc_d = df_isc_d[~((df_isc_d["Settimana"] == settimana_corrente) & (df_isc_d["Nickname"] == nick_selezionato))]
            ...
            with col_ex1:
                ...
                with pd.ExcelWriter(output_excel, engine='openpyxl') as writer:
                    df_sett.to_excel(writer, sheet_name='Deserto', index=False)
                output_excel.seek(0)
                st.download_button(label="📊 SCARICA IN FORMATO EXCEL (.xlsx)", data=output_excel, file_name=f"Iscrizioni_Deserto_{settimana_corrente}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
            with col_ex2:
                pdf_isc_data = genera_pdf_iscrizioni(df_sett, "DESERTO", settimana_corrente)
                st.download_button(label="📄 SCARICA IN FORMATO PDF (.pdf)", data=pdf_isc_data, file_name=f"Iscrizioni_Deserto_{settimana_corrente}.pdf", mime="application/pdf", use_container_width=True)
        else:
            st.warning("Nessuna iscrizione registrata per il Deserto nella settimana corrente.")
    st.stop()

# --- GESTIONE PANNELLO R4: CANYON ---
if st.session_state.get("vista_canyon_r4", False):
    st.title("🏔️ Gestione Iscrizioni - Campo di Battaglia del CANYON (R4/R5)")
    if st.button("🔙 Torna al Pannello Principale"):
        st.session_state.vista_canyon_r4 = False
        st.rerun()
    ...
    if spreadsheet_obj:
        ...
        if not df_isc_c.empty:
            df_sett = df_isc_c[df_isc_c["Settimana"] == settimana_corrente].copy() if "Settimana" in df_isc_c.columns else df_isc_c.copy()
            st.subheader("📋 Elenco Iscrizioni Registrate (Canyon)")
            
            if not df_sett.empty:
                riga_da_cancellare = st.selectbox("Seleziona iscrizione da cancellare", options=["-- Seleziona --"] + [f"{row['Nickname']} ({row['Scelta']})" for _, row in df_sett.iterrows()], key="sel_canc_canyon")
                if riga_da_cancellare != "-- Seleziona --":
                    nick_selezionato = riga_da_cancellare.split(" (")[0]
                    if st.button("🗑️ Cancella Iscrizione Selezionata", key="btn_canc_canyon"):
                        df_isc_c = df_isc_c[~((df_isc_c["Settimana"] == settimana_corrente) & (df_isc_c["Nickname"] == nick_selezionato))]
            ...
            with col_ex1:
                output_excel = io.BytesIO()
                with pd.ExcelWriter(output_excel, engine='openpyxl') as writer:
                    df_sett.to_excel(writer, sheet_name='Canyon', index=False)
                output_excel.seek(0)
                st.download_button(label="📊 SCARICA IN FORMATO EXCEL (.xlsx)", data=output_excel, file_name=f"Iscrizioni_Canyon_{settimana_corrente}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
            with col_ex2:
                pdf_isc_data = genera_pdf_iscrizioni(df_sett, "CANYON", settimana_corrente)
                st.download_button(label="📄 SCARICA IN FORMATO PDF (.pdf)", data=pdf_isc_data, file_name=f"Iscrizioni_Canyon_{settimana_corrente}.pdf", mime="application/pdf", use_container_width=True)
        else:
            st.warning("Nessuna iscrizione registrata per il Canyon nella settimana corrente.")
    st.stop()

if st.session_state.ruolo in ["Membro", "R4"]:
    is_r4_origin = (st.session_state.get("ruolo_originale") == "R4" or st.session_state.get("ruolo") == "R4")
    ...
    with st.sidebar:
        ...
        if st.session_state.get("ruolo_originale") == "R4":
            if st.button("🔙 Torna al Pannello Ufficiali R4"):
                st.session_state.ruolo = "R4"
                del st.session_state["ruolo_originale"]
                st.rerun()
        
        # Pulsanti separati Deserto e Canyon per R4 nella sidebar sotto squadre
        if st.session_state.ruolo == "R4":
            st.markdown("---")
            if st.button(T["sidebar_btn_deserto"]):
                st.session_state.vista_deserto_r4 = True
                st.rerun()
            if st.button(T["sidebar_btn_canyon"]):
                st.session_state.vista_canyon_r4 = True
                st.rerun()

            # --- CONFIGURAZIONE FINESTRA DESERTO NELLA SIDEBAR ---
            with st.expander("⏳ Finestra Deserto"):
                cfg_att_d = st.session_state.config_settimana
                try:
                    def_dt_di = datetime.datetime.strptime(cfg_att_d.get("deserto_inizio", "2026-01-01 00:00"), "%Y-%m-%d %H:%M")
                except Exception:
                    def_dt_di = datetime.datetime.now()
                try:
                    def_dt_df = datetime.datetime.strptime(cfg_att_d.get("deserto_fine", "2026-12-31 23:59"), "%Y-%m-%d %H:%M")
                except Exception:
                    def_dt_df = datetime.datetime.now() + datetime.timedelta(days=3)

                d_ini_d = st.date_input("Inizio Data", value=def_dt_di.date(), key="sb_di_d")
                d_ini_t = st.time_input("Inizio Ora", value=def_dt_di.time(), key="sb_di_t")
                d_fin_d = st.date_input("Fine Data", value=def_dt_df.date(), key="sb_df_d")
                d_fin_t = st.time_input("Fine Ora", value=def_dt_df.time(), key="sb_df_t")

                if st.button("💾 Salva Date Deserto"):
                    cfg_att_d["deserto_inizio"] = f"{d_ini_d} {d_ini_t.strftime('%H:%M')}"
                    cfg_att_d["deserto_fine"] = f"{d_fin_d} {d_fin_t.strftime('%H:%M')}"
                    st.session_state.config_settimana = cfg_att_d
                    save_config(cfg_att_d)
                    st.success("✅ Date Deserto aggiornate!")

            # --- CONFIGURAZIONE FINESTRA CANYON NELLA SIDEBAR ---
            with st.expander("⏳ Finestra Canyon"):
                cfg_att_c = st.session_state.config_settimana
                try:
                    def_dt_ci = datetime.datetime.strptime(cfg_att_c.get("canyon_inizio", "2026-01-01 00:00"), "%Y-%m-%d %H:%M")
                except Exception:
                    def_dt_ci = datetime.datetime.now()
                try:
                    def_dt_cf = datetime.datetime.strptime(cfg_att_c.get("canyon_fine", "2026-12-31 23:59"), "%Y-%m-%d %H:%M")
                except Exception:
                    def_dt_cf = datetime.datetime.now() + datetime.timedelta(days=3)

                c_ini_d = st.date_input("Inizio Data", value=def_dt_ci.date(), key="sb_ci_d")
                c_ini_t = st.time_input("Inizio Ora", value=def_dt_ci.time(), key="sb_ci_t")
                c_fin_d = st.date_input("Fine Data", value=def_dt_cf.date(), key="sb_cf_d")
                c_fin_t = st.time_input("Fine Ora", value=def_dt_cf.time(), key="sb_cf_t")

                if st.button("💾 Salva Date Canyon"):
                    cfg_att_c["canyon_inizio"] = f"{c_ini_d} {c_ini_t.strftime('%H:%M')}"
                    cfg_att_c["canyon_fine"] = f"{c_fin_d} {c_fin_t.strftime('%H:%M')}"
                    st.session_state.config_settimana = cfg_att_c
                    save_config(cfg_att_c)
                    st.success("✅ Date Canyon aggiornate!")
    ...
    if spreadsheet_obj:
        ...
        st.info(f"{T['m_month_info']} **{mese_attuale_def}** | 🗓️ Settimana Iscrizioni: **{settimana_corrente}**")
        ...
        if btn_salva_membro:
            ...
            # Corretto: se è membro richiede approvazione, se è R4 salva direttamente
            is_r4_login = (st.session_state.get("ruolo") == "R4" or st.session_state.get("ruolo_originale") == "R4")
            stato_richiesta = "Approvato" if is_r4_login else ("In Attesa (⚠️ Duplicati)" if duplicati_presenti else "In Attesa")
        ...
        # --- MODULO ISCRIZIONE DESERTO ---
        st.markdown("---")
        st.subheader("🏜️ Modulo Iscrizione - Battaglia del Deserto")
        
        cfg_corrente = st.session_state.config_settimana
        try:
            dt_inizio_d = datetime.datetime.strptime(cfg_corrente.get("deserto_inizio", "2026-01-01 00:00"), "%Y-%m-%d %H:%M")
            dt_fine_d = datetime.datetime.strptime(cfg_corrente.get("deserto_fine", "2026-12-31 23:59"), "%Y-%m-%d %H:%M")
        except Exception:
        ...
        st.info(f"⏳ Finestra Deserto: dal **{dt_inizio_d.strftime('%d/%m/%Y %H:%M')}** al **{dt_fine_d.strftime('%d/%m/%Y %H:%M')}**")
        if not deserto_aperto:
            st.error("🔴 **Iscrizioni Deserto CHIUSE.** Fuori tempo massimo.")
        ...
        with st.form("form_iscrizione_deserto"):
        ...
        # --- MODULO ISCRIZIONE CANYON ---
        st.markdown("---")
        st.subheader("🏔️ Modulo Iscrizione - Battaglia del Canyon")
        
        try:
            dt_inizio_c = datetime.datetime.strptime(cfg_corrente.get("canyon_inizio", "2026-01-01 00:00"), "%Y-%m-%d %H:%M")
            dt_fine_c = datetime.datetime.strptime(cfg_corrente.get("canyon_fine", "2026-12-31 23:59"), "%Y-%m-%d %H:%M")
        except Exception:
            dt_inizio_c, dt_fine_c = datetime.datetime.min, datetime.datetime.max

        canyon_aperto = dt_inizio_c <= now_utc <= dt_fine_c

        st.info(f"⏳ Finestra Canyon: dal **{dt_inizio_c.strftime('%d/%m/%Y %H:%M')}** al **{dt_fine_c.strftime('%d/%m/%Y %H:%M')}**")
        if not canyon_aperto:
            st.error("🔴 **Iscrizioni Canyon CHIUSE.** Fuori tempo massimo.")
        ...
        with st.form("form_iscrizione_canyon"):
...
if st.session_state.ruolo == "R4":
        ...
        with st.form("form_wizard_mod"):
            ...
            if btn_salva_wiz:
                new_config = {
                    ...
                    "deserto_inizio": cfg_old.get("deserto_inizio", "2026-01-01 00:00"),
                    "deserto_fine": cfg_old.get("deserto_fine", "2026-12-31 23:59"),
                    "canyon_inizio": cfg_old.get("canyon_inizio", "2026-01-01 00:00"),
                    "canyon_fine": cfg_old.get("canyon_fine", "2026-12-31 23:59")
                }
    ...
    with st.sidebar:
        ...
        with st.expander(T["sidebar_reset_title"]):
            ...
            if st.button(T["sidebar_reset_btn"], type="primary", use_container_width=True):
                    ...
                    try:
                        ...
                        if client_res:
                            ...
                            try:
                                ...
                                pass
                            try:
                                for ws_name in ["Registro_Deserto", "Registro_Canyon"]:
                                    sh_res.worksheet(ws_name).clear()
                            except Exception:
                                pass
    ...
    with tab_report:
        ...
        if df_drive is not None:
            ...
            if "df_calcolato" in st.session_state:
                ...
                st.download_button(label=T["download_pdf_btn"], data=pdf_data, file_name="Report.pdf", mime="application/pdf", type="primary", use_container_width=True)
