...
TEXTS = {
    "🇮🇹 🇮🇹 Italiano": {
        ...
        * **VS** = Punti VS inseriti (numeri interi puri)  
        * **Comb.** = Combattente Sabato (si/no)  
        * **Tech** = Donazioni Tech inserite (numeri interi puri)  
...
def genera_report_pdf(df, tipo_settimana):
    ...
    for pos, (_, row) in enumerate(df_sorted.iterrows(), start=1):
        ...
        table_data.append([
            ...
            f"{int(converti_in_numero_puro(row.get('Punti_VS', 0))):,}".replace(",", "."),
            f"{int(converti_in_numero_puro(row.get('Donazioni_Tech', 0))):,}".replace(",", "."),
            str(int(converti_in_numero_puro(row.get('Premi_Stella', 0)))),
