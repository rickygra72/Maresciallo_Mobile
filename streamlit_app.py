...
TEXTS = {
    "🇮🇹 🇮🇹 Italiano": {
        ...
        * **VS** = Punti VS inseriti (formattati con punti migliaia, es. 86.234.091)  
        * **Comb.** = Combattente Sabato (si/no)  
        * **Tech** = Donazioni Tech inserite (formattate con punti migliaia, es. 10.000)  
        * **Stelle** = Premi Stella (0 a 5)  
...
# --- FUNZIONI DI CONVERSIONE E FORMATTAZIONE CON PUNTINO ---
    ...
    try:
        ...
        if client:
            ...
            if rows and len(rows) > 0:
                ...
                # Applicazione visiva dei puntini per VS e Tech direttamente come stringa formattata
...
def salva_dati_su_drive(sheet, df):
    try:
        ...
        if client_fresco:
            ...
            # Pulisce rigorosamente rimuovendo puntini e salvando solo interi puri
...
# ==============================================================================
# 3. TAB DATI GOOGLE DRIVE (CON MENU A TENDINA STELLE E FORMATTAZIONE PUNTINI)
# ==============================================================================
with tab_dati:
    ...
    if df_drive is not None:
        ...
        for idx, row in df_drive.iterrows():
            ...
            vs_str = str(converti_in_numero_puro(row.get("Punti_VS", 0)))
            tech_str = str(converti_in_numero_puro(row.get("Donazioni_Tech", 0)))
            
            if vs_str != "0" and len(vs_str) < 7:
                anomalie_rilevate.append(f"⚠️ **{nick_chk}**: Valore VS (`{row.get('Punti_VS')}`) troppo corto.")
            if tech_str != "0" and len(tech_str) < 4:
                anomalie_rilevate.append(f"⚠️ **{nick_chk}**: Donazioni Tech (`{row.get('Donazioni_Tech')}`) troppo corte.")

        if anomalie_rilevate:
            with st.expander("🚨 **AVVISI DI SICUREZZA R4**", expanded=True):
        ...
        opzioni_stelle = ["0", "1", "2", "3", "4", "5"]
        
        # Converte Premi_Stella in stringa per allinearlo correttamente al SelectboxColumn
        if "Premi_Stella" in df_drive.columns:
            df_drive["Premi_Stella"] = df_drive["Premi_Stella"].astype(str)
        ...
        column_configuration = {
            ...
            "Premi_Stella": st.column_config.SelectboxColumn(T["col_premi"], options=opzioni_stelle, width="small"),
