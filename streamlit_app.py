...
TEXTS = {
    "🇮🇹 🇮🇹 Italiano": {
        ...
        * **VS** = Punti VS inseriti (formattati con separatore migliaia)  
        * **Comb.** = Combattente Sabato (si/no)  
        * **Tech** = Donazioni Tech inserite (formattate con separatore migliaia)  
        * **Stelle** = Premi Stella (tramite menu a tendina)  
...
# --- FUNZIONI DI CONVERSIONE E PULIZIA NUMERICA PURA ---
...
# --- GOOGLE DRIVE CONNECTION (SENZA CACHE BLOCCANTE) ---
    ...
    try:
        ...
        if client:
            ...
            if rows and len(rows) > 0:
                ...
                # Conversione in interi puri
                for col_f in ["Punti_VS", "Donazioni_Tech", "Premi_Stella"]:
                    if col_f in df.columns:
                        df[col_f] = df[col_f].apply(converti_in_numero_puro)
...
def genera_report_pdf(df, tipo_settimana):
    ...
    for pos, (_, row) in enumerate(df_sorted.iterrows(), start=1):
        ...
        table_data.append([
            ...
            f"{int(row.get('Punti_VS', 0)):,}".replace(",", "."),
            f"{int(row.get('Donazioni_Tech', 0)):,}".replace(",", "."),
            str(int(row.get('Premi_Stella', 0))),
...
# ==============================================================================
# 3. TAB DATI GOOGLE DRIVE (PUNTINI MIGLIAIA + MENU A TENDINA STELLE)
# ==============================================================================
with tab_dati:
    ...
    if df_drive is not None:
        ...
        column_configuration = {
            T["col_n"]: st.column_config.NumberColumn(T["col_n"], width="small", disabled=True),
            "Nickname": st.column_config.TextColumn(T["col_nickname"], width="medium", required=True),
            "Punti_VS": st.column_config.NumberColumn(T["col_punti_vs"], format="%,d", width="small"),
            "Combattente_Sabato": st.column_config.SelectboxColumn(T["col_combattente"], options=opzioni_yes_no, width="small"),
            "Donazioni_Tech": st.column_config.NumberColumn(T["col_donazioni"], format="%,d", width="small"),
