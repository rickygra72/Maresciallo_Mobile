...
# --- FUNZIONI DI NORMALIZZAZIONE INPUT (Pulisce M e K e tiene solo numeri puri) ---
def normalizza_valore_vs(val_str):
    if val_str is None or str(val_str).strip() == "" or str(val_str).strip() == "0":
        return "0"
    digs = ''.join(filter(str.isdigit, str(val_str).upper().replace('M', '').replace('K', '')))
    if not digs:
        return "0"
    return str(int(digs))
...
def carica_dati_da_drive():
    ...
    if rows and len(rows) > 0:
        ...
        # Formattazione visiva con puntino delle migliaia per la tabella (senza M o K)
...
def salva_dati_su_drive(sheet, df):
    try:
        ...
        if client_fresco:
            ...
            # Pulisce definitivamente rimuovendo puntini, M, K e tenendo solo i numeri puri
            for col_f in ["Punti_VS", "Donazioni_Tech"]:
                if col_f in df_pulito.columns:
                    df_pulito[col_f] = df_pulito[col_f].apply(lambda x: ''.join(filter(str.isdigit, str(x).upper().replace('M', '').replace('K', ''))) or "0")
