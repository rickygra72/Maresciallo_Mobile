...
TEXTS = {
    "🇮🇹 🇮🇹 Italiano": {
        ...
        "confirm_config": "💾 Salva e Torna alla Dashboard", "tab_dati": "📊 Inserimento Punteggi Sicuro",
        ...
        "db_legend_text": "Gestisci i dati dei giocatori in modo sicuro e pulito dai moduli sottostanti.",
    ...
    "🇩🇪 🇩🇪 Deutsch": {
        ...
        "col_donazioni": "Tech", "col_premi": "Sterne", "col_pen_scudo": "Schild", "col_assenza": "Abwes.", "col_mancata_risp": "Keine R4",
        "col_inattivita": "Inaktiv", "col_punteggio_tot": "Gesamtpunkte", "col_pot_tot": "Gesamtstärke", "col_growth_tot": "Gesamtwachstum %"
    }
}
    ...
    if spreadsheet_obj:
        ...
        if btn_salva_membro:
            ...
            if df_squadre[maschera].shape[0] > 0:
                ...
                df_squadre_agg = pd.concat([df_squadre, nueva_riga] if 'nueva_riga' in locals() else [df_squadre, nuova_riga], ignore_index=True)
...
# ==============================================================================
# 3. TAB DATI GOOGLE DRIVE (FORM NATIVA SICURA)
# ==============================================================================
with tab_dati:
    st.subheader("📊 Inserimento Punteggi (Modulo Sicuro Senza Bug)")
    ...
    if df_drive is not None:
        ...
        with st.form("form_inserimento_sicuro"):
            ...
            for idx, row in df_drive.iterrows():
                ...
                with c1:
                    vs_val_pulito = formatta_con_puntini(row.get("Punti_VS", 0))
                    if vs_val_pulito == "0": vs_val_pulito = ""
                    inp_vs = st.text_input(f"Punti VS ({nick})", value=vs_val_pulito, key=f"vs_form_{idx}")
                with c2:
                    tech_val_pulito = formatta_con_puntini(row.get("Donazioni_Tech", 0))
