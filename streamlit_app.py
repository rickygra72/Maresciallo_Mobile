...
TEXTS = {
    "🇮🇹 🇮🇹 Italiano": {
        ...
        "wizard_subtitle": "Imposta i parametri tattici per il calcolo dei punteggi di questa settimana (inserisci i numeri interi reali, es. 100000000 per 100M).",
        ...
        "m_format_info": "💡 Inserisci il valore numerico intero della potenza.",
        ...
        * **VS** = Valore numerico intero dei punti VS (es. 86212130)  
        * **Comb.** = Combattente Sabato (si/no)  
        * **Tech** = Valore numerico delle donazioni Tech (es. 10000)  
        * **Stelle** = Premi Stella  
    ...
    "🇬🇧 🇬🇧 English": {
        ...
        "m_format_info": "💡 Enter the raw integer power value.",
...
def normalizza_valore_vs(val_str):
    if val_str is None or str(val_str).strip() == "" or str(val_str).strip() == "0":
        return "0"
    return ''.join(filter(str.isdigit, str(val_str)))
...
def parse_compact_number(val_str, default_val=0.0):
    ...
    digs = ''.join(filter(str.isdigit, str(val_str)))
    if not digs:
        return default_val
    try:
        return float(digs)
...
with tab_dati:
    ...
    if df_drive is not None:
        ...
        for idx, row in df_drive.iterrows():
            ...
            vs_chk = str(row.get("Punti_VS", "")).strip()
            tech_chk = str(row.get("Donazioni_Tech", "")).strip()
            
            # Controlliamo se il VS ha meno di 7 cifre (e non è zero)
            if vs_chk and vs_chk != "0" and len(vs_chk) < 7:
                anomalie_rilevate.append(f"⚠️ **{nick_chk}**: Il valore VS (`{vs_chk}`) sembra troppo corto (attese 7-9 cifre).")
            # Controlliamo se le donazioni Tech hanno meno di 4 cifre (e non sono zero)
            if tech_chk and tech_chk != "0" and len(tech_chk) < 4:
                anomalie_rilevate.append(f"⚠️ **{nick_chk}**: Le donazioni Tech (`{tech_chk}`) sembrano troppo corte (attese 4-5 cifre).")
